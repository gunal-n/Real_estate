import React from 'react'
import Navbar from './Navbar'


const Header = () => {
  return (
    <div className='flex items-center w-full min-h-screen overflow-hidden bg-center bg-cover'
    style={{backgroundImage:"url('/header_img.jpg')"}}
    id='home'
    >
        <Navbar/>

    <div className="container px-6 py-4 mx-auto text-center text-white md:px-20 lg:px-32">
       <h2 className='text-5xl sm:text-6xl md:text-[72px] max-w-3xl  inline-block pt-20'>Explore homes that fit your dreams</h2>
       <div className='mt-16 space-x-6'>
        <button className='px-8 py-3 border rounded hover:text-gray-400'><a href='#'>Projects</a></button>
        <button className='px-8 py-3 bg-blue-500 border rounded'><a href='#'> Contact us </a></button>
       </div>
    </div>
    </div>
  )
}

export default Header